...
(() => {
    ...
    const websocket = {
        ...
        connect: function (host) {
            ...
            this.socket.onopen = function () {
                ...
                console.log('Info: WebSocket connection openedesesss.');
            };
        ...
        initialize: function () {
           // const url = "ws://" + location.host + location.pathname.substr(0, location.pathname.lastIndexOf('/')) + "/ws/ms?devId=web_view";
    ...
    const store = {
        db: {
            itensContentPageMenu:[],
            itensContentPageVersion:null,
            menu: {
                tarefas: []
            },
            preferencias: null,
    ...
    const service = {
        ...
        getMenuTarefasV3(idprocesso,idGrupoUsuario) {
            return this.transport.get({
                url: "odwctrl?action=execTarefa&apelido=SYSTEM_menu_v3&idprocesso="+idprocesso+"&idGrupoUsuarios="+idGrupoUsuario,
                data: {
                    select: "menuv3.idtarefa, menuv3.desctarefa, menuv3.apelido, menuv3.icone"
                }
            });
        },
        getPreferencias() {
            return this.transport.get({
                ...
                data: {
                    select: "idPreferencia,tipoHierarquiaMenu,apelidoTarefaMenu,dataHoraVisualExecMilis,dataHoraVisualNotifMilis,usuario.idUsuario,usuario.usuario,usuario.nomeUsuario, usuario.grupoUsuarios.idGrupoUsuarios, usuario.grupoUsuarios.descGrupoUsuarios"
                }
            });
        },
        getVisaoInicial() {
    ...
    const view = {
        "sidebar-left": {
            ...
            methods: {
                ...
                //     console.log(tarefa.descTarefa);
                //     document.getElementById('labelcard').innerText = tarefa.descTarefa;
                // },
                open(idprocesso,tarefa,index) {                    
                    this.$$bus.$emit("Content.open", idprocesso);
                    this.activeIndex = index;
                    document.getElementById('labelcard').innerText = tarefa.descTarefa;
                },
                ...
                openSearch(event) {
                    ...
                    if (tarefa) {
                        event.target.value = "";
                        this.open(idprocesso,tarefa);
                    } 
                },
                menuTarefas() {
                    return this.$$store.db.menu.tarefas;
                },
                // getMenuTarefasV3(idprocesso) {
                //     const idGrupoUsuario = document.getElementById("idGrupo").textContent;
                //     service.getMenuTarefasV3(idprocesso,idGrupoUsuario)
                //         .then((response) => {                            
                //             this.$$store.db.itensContentPageMenu = response.menuv3;
                //             document.getElementById("conteudoPagina").textContent = response.menuv3;                     
                //         })
                // },
        ...
        "content": {
            ...
            created() {
                ...
                //     });
                // });
                this.$$bus.$on("Content.open", (idprocesso) => {
                    const idGrupoUsuario = document.getElementById("idGrupo").textContent;
                    service.getMenuTarefasV3(idprocesso,idGrupoUsuario)
                        .then((response) => {                            
                            this.$$store.db.itensContentPageMenu = response.menuv3;
                            this.$$store.db.itensContentPageVersion = response.pacotes;
                            this.itensContentPageMenu = this.$$store.db.itensContentPageMenu;
                            this.itensContentPageVersion = 'Pacotes: ' + this.$$store.db.itensContentPageVersion;
                            $("#footer_version").text(this.itensContentPageVersion); 
                            $("#navbar").show();
                            $(".footer").show();    
                ...
                this.$$bus.$on("Content.openNotificacao", (notificacao) => {
                    const url = notificacao.tarefaExecucao.idTarefaExecucao + '&hash=' + notificacao.tarefaExecucao.hash;
                    this.new({
...
//$$bus.on escuta $$bus.emit emite => Comunicação entre componentes
            mounted() {
                // this.$nextTick(function () {
                ...
                // })
                // this.$nextTick(function () {
                //     const tarefasDoMenu = this.$$store.db.itensContentPageMenu;
                //     this.itensContentPageMenu = this.$$store.db.itensContentPageMenu;
                //     console.log(tarefasDoMenu);
                //     let vi;
                //     for (let i in tarefasDoMenu) {
                //         vi = tarefasDoMenu[i];
                //         if (vi.tipoItemDesc == "Tarefa") {
                //             const tarefa = this.$$store.db.getTarefaByApelido(vi.idItem);
                //             if (tarefa) {
        ...
        "layout":{
            mounted() {
                document.body.classList.add(`semi-dark-layout`);
                const nav = document.getElementById('navbar');
                nav.classList.add(`navbar-semi-dark`);
            },
            methods:{
                mudaTema (tema){
                    document.body.classList.remove(`semi-dark-layout`,`dark-layout`);
                    document.body.classList.add(tema+'-layout');
                    const nav = document.getElementById('navbar');
                    nav.classList.remove(`navbar-semi-dark`,`navbar-dark`);
                    nav.classList.add(`navbar-`+tema);
                }
            }
        },
        init() {
